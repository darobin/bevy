...
function updateApp (app, cb) {
    ...
    if (!fs.existsSync(app.contentPath)) fs.mkdirSync(app.contentPath);
    ...
    fs.writeFileSync(app.configPath, "utf8", JSON.stringify(app));
...
function startAPI (port) {
    ...
    app.put("/app/:name", function (req, res) {
        ...
        desc.contentPath = pth.join(desc.storePath, desc.repository.type);
    ...
    app.del("/app/:name", pickApp, function (req, res) {
        stopApp(req.bevyApp, function (err) {
