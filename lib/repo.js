...
function npmInstall (app, conf, session, cb) {
    ...
    // check that this actually requires npm
    if (!fs.existsSync(pth.join(app.contentPath, "package.json"))) {
        session.progress("No package.json, npm not required.\n").end();
        return cb();
    }
...
function git (app, conf, session, cb) {
    fs.exists(app.contentPath, function (exists) {
        ...
        if (exists) {
            ...
            commands.push(["git", ["clone", "-b", branch, app.repository.url, "content"], app.storePath]);
        }
