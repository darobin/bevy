...
function Bevy (conf) {
    ...
    // chown if we're running with different users
    function chownIfNeeded (path) {
        if (conf.uid || conf.gid) fs.chownSync(path, conf.uid, conf.gid);
    }
    
    // create a directory, with proper ownership, if it's needed
    ...
    // write a file, taking perms into account
    function writeFileOwned (path, content) {
    ...
    function startChild (childConf) {
        var outLog = pth.join(childConf.logPath, "access.log")
        ,   errLog = pth.join(childConf.logPath, "error.log")
        ,   foreverConf = {
            env:        {
                PORT:           childConf.port
        ...
        if (!fs.existsSync(childConf.cwd)) {
            console.error("[ERROR] Path (cwd) for app does not exist: " + childConf.cwd);
            return;
        }
    ...
    function updateApp (app, cb) {
        ...
        writeFileOwned(app.configPath, JSON.stringify(app, null, 4));
    ...
    function startAPI (port) {
        ...
        // generate a form for the UI
        // var cachedForm;
        // app.get("/app-form.html", function (req, res) {
        //     if (!cachedForm) {
        //         var wsf = require("web-schema-forms")
        //         ,   bs = require("web-schema-forms-bootstrap")
        //         ,   schema = JSON.parse(fs.readFileSync(pth.join(__dirname, "../schema/bevy-app.json"), "utf8"))
        //         ;
        //         wsf.form({
        //                 extensions: [bs.horizontal]
        //             ,   schema:     schema
        //             }
        //         ,   function (err, form) {
        //                 if (err) return res.send("<span style='color: red;'>Form generation error: " + err + "</span>");
        //                 cachedForm = form;
        //                 res.send(form);
        //             }
        //         );
