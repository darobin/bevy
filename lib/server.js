...
function Bevy (conf) {
    ...
    function startChild (childConf) {
        ...
        if (conf.uid) foreverConf.spawnWith.uid = { uid: conf.uid };
        if (conf.gid) foreverConf.spawnWith.gid = { gid: conf.gid };
        return new forever.Monitor(conf.startPath, foreverConf);
    }
    ...
    function spawnApp (app, cb) {
        getPort(function (err, port) {
            ...
            if (app.static) {
                ...
                childConf.cwd = (app.repository.type === "local") ? app.repository.path : app.contentPath;
            }
            else {
