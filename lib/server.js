...
function Bevy (conf) {
    ...
    function spawnApp (app, cb) {
        getPort(function (err, port) {
            ...
            if (app.static) {
                conf.startPath = staticServerPath;
                conf.cwd = (app.repository.type === "local") ? app.repository.path : app.contentPath;
            }
            else {
                conf.startPath = app.startPath;
                conf.cwd = app.contentPath;
            }
    ...
    function startAPI (port) {
        ...
        app.get("/session/:id", function (req, res) {
            ...
            ,   session = sessions[id]
            ;
            if (!session) return res.send(404, "No such running session.");
