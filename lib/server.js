...
,   staticServerPath = pth.join(__dirname, "static-server.js")
    // XXX the below is WTF, also use an ENV
,   configPath = pth.join(process.cwd(), process.argv[2]) || "/etc/bevy/config.json"
;

// XXX the below shouldn't just load, it should act like a library
...
function startChild (conf) {
    return new forever.Monitor(conf.startPath, {
        env:        { PORT: conf.port, BEVY_ROOT: conf.root || conf.cwd }
    ,   cwd:        conf.cwd
    ,   logFile:    pth.join(conf.cwd, "forever.log")
    ,   outFile:    pth.join(conf.cwd, "access.log")
    ,   errFile:    pth.join(conf.cwd, "error.log")
    });
}
...
function spawnApp (app, cb) {
    getPort(function (err, port) {
        ...
        if (app.static) {
            ...
            ,   port:       port
            ,   cwd:        app.contentPath
            ,   root:       (app.repository.type === "local") ? app.repository.path : app.contentPath
            };
        }
        else {
            conf = {
